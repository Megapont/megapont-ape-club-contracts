...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let block = chain.mineBlock([
      Tx.contractCall('megapont-ape-club-nft', 'get-last-token-id', [], wallet_1.address),
      Tx.contractCall('megapont-ape-club-nft', 'get-token-uri', [types.uint(1)], wallet_1.address),
    ]);
